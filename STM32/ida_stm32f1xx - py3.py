...
def CreateEnum(Regs, BaseAddr, EnumName):
    ...
    ida_enum.del_enum(ida_enum.get_enum(EnumName))

    EnumId = idc.add_enum(0, EnumName, ida_bytes.hex_flag())

    for Reg, Descr in Regs.items():
        Comment = None
        if isinstance(Descr, (int, int)):
            Offset = Descr
        else:
            Offset = Descr[0]
            Comment = Descr[1]
...
def CreateVecTable(TableSize=None):
    ...
    for Vector in Vectors:
        ...
        ida_bytes.del_items(ea, DELIT_SIMPLE, 4)
        idc.create_dword(ea)
